...
namespace MicrosoftOpenXR
{
	...
	void FLocatableCamPlugin::StartCameraCapture(int DesiredWidth, int DesiredHeight, int DesiredFPS)
	{
		...
		FindAllAsyncOp.Completed([this, DesiredWidth, DesiredHeight, DesiredFPS](auto&& asyncInfo, auto&& asyncStatus)
		{
			std::lock_guard<std::recursive_mutex> lock(RefsLock);
			...
			InitializeAsyncOp.Completed([this, Capture, ChosenSourceInfo](auto&& asyncInfo, auto&& asyncStatus)
			{
				std::lock_guard<std::recursive_mutex> lock(RefsLock);
				if (asyncStatus == winrt::Windows::Foundation::AsyncStatus::Canceled)
				{
				...
				CreateFrameReaderAsyncOp.Completed([this, Capture, FrameSource](auto&& asyncInfo, auto&& asyncStatus)
				{
					std::lock_guard<std::recursive_mutex> lock(RefsLock);
					if (asyncStatus == winrt::Windows::Foundation::AsyncStatus::Canceled)
					{
					...
					StartAsyncOp.Completed([this, Capture, FrameReader, FrameSource](auto&& asyncInfo, auto&& asyncStatus)
					{
						std::lock_guard<std::recursive_mutex> lock(RefsLock);
